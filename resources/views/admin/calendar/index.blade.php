...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
 ...
 <div class="px-2 py-2.5 text-center text-xs font-semibold text-gray-400 uppercase">{{ $day }}</div>
 @endforeach
 </div>
 {{-- Calendar Days --}}
 @php
 ...
 <div class="min-h-[80px] p-1.5 border-b border-gray-100 {{ $isToday ? 'bg-brand-50/50 ' : 'hover:bg-gray-50 ' }} transition-colors">
 <p class="text-xs font-semibold {{ $isToday ? 'w-6 h-6 rounded-full bg-brand-500 text-white flex items-center justify-center' : 'text-gray-500 ' }} mb-1">{{ $d }}</p>
 {{-- Event dots (placeholder) --}}
 @if($d % 7 == 0 || $d % 11 == 0)
 <div class="px-1 py-0.5 rounded text-xs bg-brand-100 text-brand-700 truncate mb-0.5">Seminar</div>
 @endif
 </div>
 @endfor
 </div>
 ...
 <h3 class="text-sm font-semibold text-gray-700 mb-3">Add Event</h3>
 <div class="space-y-3">
 <input type="text" placeholder="Event title" disabled
 class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-transparent text-sm text-gray-900 placeholder-gray-400 opacity-60">
 <input type="date" disabled
 class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-transparent text-sm text-gray-900 opacity-60">
 <select disabled class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-white text-sm text-gray-700 opacity-60">
