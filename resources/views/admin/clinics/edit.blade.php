<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex items-center space-x-3 mt-4">
            <a href="{{ route('admin.clinics.show', $clinic) }}" class="text-gray-600 hover:text-gray-900">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
            </a>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit {{ $clinic->name }}</h2>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.clinics.update', $clinic) }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="type" id="type" required
                                        ...
                                        <option value="Clinic" {{ old('type', $clinic->type?->value) == 'Clinic' ? 'selected' : '' }}>Clinic</option>
                                        <option value="Testing Center" {{ old('type', $clinic->type?->value) == 'Testing Center' ? 'selected' : '' }}>Testing Center</option>
                                        <option value="Hospital" {{ old('type', $clinic->type?->value) == 'Hospital' ? 'selected' : '' }}>Hospital</option>
                                        <option value="Barangay Health Station" {{ old('type', $clinic->type?->value) == 'Barangay Health Station' ? 'selected' : '' }}>Barangay Health Station</option>
                                    </select>
                                    @error('type')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <select name="city" id="city" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        @foreach($cities as $city)
                                            <option value="{{ $city->name }}" data-code="{{ $city->code }}" {{ old('city', $clinic->city) == $city->name ? 'selected' : '' }}>{{ $city->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="barangay" id="barangay" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        @if(old('barangay', $clinic->barangay))
                                            <option value="{{ old('barangay', $clinic->barangay) }}" selected>{{ old('barangay', $clinic->barangay) }}</option>
                                        @endif
                                    </select>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="address" id="address" rows="3" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('address', $clinic->address) }}</textarea>
                                    @error('address')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div>
                                ...
                                <select name="operating_hours" id="operating_hours" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    <option value="Mon to Fri 8:00 AM - 5:00 PM" {{ old('operating_hours', $clinic->operating_hours) == 'Mon to Fri 8:00 AM - 5:00 PM' ? 'selected' : '' }}>Mon to Fri 8:00 AM - 5:00 PM</option>
                                    <option value="Mon to Sat 8:00 AM - 5:00 PM" {{ old('operating_hours', $clinic->operating_hours) == 'Mon to Sat 8:00 AM - 5:00 PM' ? 'selected' : '' }}>Mon to Sat 8:00 AM - 5:00 PM</option>
                                    <option value="Mon to Sun 8:00 AM - 5:00 PM" {{ old('operating_hours', $clinic->operating_hours) == 'Mon to Sun 8:00 AM - 5:00 PM' ? 'selected' : '' }}>Mon to Sun 8:00 AM - 5:00 PM</option>
                                    <option value="24/7" {{ old('operating_hours', $clinic->operating_hours) == '24/7' ? 'selected' : '' }}>24/7</option>
                                    <option value="Mon to Fri 9:00 AM - 6:00 PM" {{ old('operating_hours', $clinic->operating_hours) == 'Mon to Fri 9:00 AM - 6:00 PM' ? 'selected' : '' }}>Mon to Fri 9:00 AM - 6:00 PM</option>
                                    <option value="Mon to Sat 9:00 AM - 6:00 PM" {{ old('operating_hours', $clinic->operating_hours) == 'Mon to Sat 9:00 AM - 6:00 PM' ? 'selected' : '' }}>Mon to Sat 9:00 AM - 6:00 PM</option>
                                </select>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="contact" id="contact" value="{{ old('contact', $clinic->contact) }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('contact')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                @php
                                    $currentServices = old('services', $clinic->services ?? []);
                                @endphp
                                @foreach(\App\Enums\ClinicService::cases() as $service)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="services[]" value="{{ $service->value }}" 
                                               ...
                                               {{ in_array($service->value, $currentServices) ? 'checked' : '' }}>
                                ...
                                <div>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="verified" value="1" 
                                               class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                               {{ old('verified', $clinic->verified) ? 'checked' : '' }}>
                        ...
                        <div class="mb-8">
                            ...
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="4"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                    placeholder="Any additional notes or special instructions...">{{ old('notes', $clinic->notes) }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div>
                                ...
                                <textarea name="rejection_reason" id="rejection_reason" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">{{ old('rejection_reason', $clinic->rejection_reason) }}</textarea>
    ...
    <script>
        ...
        function initializeMap() {
            ...
            // If we have old values (form validation failed) or existing clinic location, show them
