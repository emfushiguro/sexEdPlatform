...
<div class="mx-auto max-w-7xl px-4 py-8"
    ...
    <div :class="(activeReview !== null || documentPreviewOpen) ? 'blur-[2px] scale-[0.995] pointer-events-none select-none' : ''" class="space-y-8 transition duration-300 ease-out">
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 mb-6">
            <article class="min-h-[116px] rounded-[28px] border border-brand-100 bg-gradient-to-br from-white via-brand-50/70 to-brand-100/60 p-5 shadow-theme-xs">
                ...
                <p class="mt-3 text-4xl leading-none font-bold text-gray-900">{{ number_format($pendingCount) }}</p>
            </article>
            <article class="min-h-[116px] rounded-[28px] border border-brand-200 bg-gradient-to-br from-brand-100/60 via-white to-brand-50 p-5 shadow-theme-xs">
                ...
                <p class="mt-3 text-4xl leading-none font-bold text-gray-900">{{ number_format($approvedCount) }}</p>
            </article>
            <article class="min-h-[116px] rounded-[28px] border border-brand-300 bg-gradient-to-br from-brand-100 via-white to-brand-200/70 p-5 shadow-theme-xs">
                ...
                <p class="mt-3 text-4xl leading-none font-bold text-gray-900">{{ number_format($rejectedCount) }}</p>
            </article>
        </section>
        ...
        <section class="overflow-hidden rounded-[30px] border border-gray-200 bg-white shadow-theme-xs">
            <div class="border-b border-brand-100 bg-[radial-gradient(circle_at_top_left,_rgba(163,14,178,0.17),_transparent_34%),radial-gradient(circle_at_top_right,_rgba(59,12,177,0.14),_transparent_32%),linear-gradient(180deg,#ffffff_0%,#f8f3ff_100%)] px-6 py-6">
                <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                    ...
                    <form method="GET" action="{{ route('admin.instructor-applications.index') }}" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6" data-testid="admin-table-filter-bar">
                        ...
                        <label class="block">
                            ...
                            <select name="status" onchange="this.form.submit()" class="w-full px-4 py-3 text-sm text-gray-900 transition bg-white border border-brand-100 shadow-sm outline-none rounded-2xl focus:border-gray-300 focus:ring-2 focus:ring-gray-100">
                        ...
                        {{-- Empty cols to push things if needed, or allow other filters to expand --}}
                        ...
                        @forelse($applications as $application)
                            @php
                                ...
                                $actionButtonClass = $application->status === 'approved'
                                    ...
                                    : ($application->status === 'rejected' ? 'border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100' : 'border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100');
    ...
    <div x-show="confirmOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center px-4" @keydown.escape.window="closeActionModal()">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div class="mt-6 flex items-center justify-end gap-2">
                <button type="button" @click="closeActionModal()" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-brand-50/45">Cancel</button>
                <button type="button"
                        @click="submitAction()"
            ...
            <div x-show="activeReview === {{ $application->id }}"
                                    ...
                                    @foreach($tierOneDocuments as $document)
                                        @if(! empty($document['path']))
                                            @php
                                                ...
                                                $isPdf = $extension === 'pdf';
                                            @endphp
                                            <div class="rounded-xl border border-gray-200 bg-brand-50/45/60 p-4">
                                                ...
                                                <div class="mt-3 flex gap-2">
                                                    ...
                                                    <a href="{{ $documentUrl }}" download class="inline-flex items-center justify-center w-10 h-10 transition border rounded-2xl border-brand-200 bg-white hover:bg-brand-50 text-gray-700" title="Download Document" aria-label="Download Document">
                                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <section class="rounded-2xl border border-gray-200 bg-white p-5" x-data="{ open: true }">
                        ...
                        <div x-show="open" x-cloak class="mt-4">
                            @if($application->reviews->isEmpty())
                                <p class="rounded-xl border border-gray-200 bg-brand-50/45 p-4 text-sm text-gray-600">No moderation actions have been recorded yet.</p>
                            @else
                                <div class="space-y-3">
                                    @foreach($application->reviews as $review)
                                        <article class="rounded-xl border border-gray-200 bg-brand-50/45/60 p-4">
                ...
                <div x-show="approveModalOpen"
                    ...
                    <div class="w-full max-w-xl rounded-2xl border border-gray-200 bg-white shadow-2xl overflow-hidden">
                        <div class="border-b border-gray-100 bg-brand-50/45/50 px-6 py-5">
                        ...
                        <form method="POST" action="{{ route('admin.instructor-applications.approve', $application) }}" @submit="syncModerationEditors()">
                            ...
                            <div class="flex items-center justify-end gap-3 border-t border-gray-100 bg-brand-50/45/50 px-6 py-4">
                                <button type="button"
                                        class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-brand-50/45 transition-colors"
                    ...
                    <div class="w-full max-w-xl rounded-2xl border border-gray-200 bg-white shadow-2xl overflow-hidden">
                        ...
                        <form method="POST" action="{{ route('admin.instructor-applications.reject', $application) }}" @submit="syncModerationEditors()" class="space-y-4 px-6 py-5">
                            ...
                            <div class="flex items-center justify-end gap-2 border-t border-gray-100 pt-4">
                                <button type="button"
                                        class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-brand-50/45"
    ...
    <div x-show="documentPreviewOpen"
        ...
        <div x-show="documentPreviewOpen"
            ...
            <div class="border-b border-gray-100 bg-brand-50/45/80 px-6 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-brand-600">Document Preview</p>
            ...
            <div class="max-h-[80vh] space-y-4 overflow-y-auto bg-white px-6 py-6">
                ...
                <template x-if="documentPreviewType === 'file'">
                    <div class="rounded-xl border border-gray-200 bg-brand-50/45 p-5 text-sm text-gray-600">
