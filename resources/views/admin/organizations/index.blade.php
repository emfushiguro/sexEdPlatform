...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
    @foreach([['label'=>'Partner Orgs','value'=>'28','bg'=>'bg-brand-50 dark:bg-brand-500/10','color'=>'text-brand-600 dark:text-brand-400'],['label'=>'Schools','value'=>'12','bg'=>'bg-purple-50 dark:bg-purple-500/10','color'=>'text-purple-600 dark:text-purple-400'],['label'=>'Clinics / NGOs','value'=>'10','bg'=>'bg-success-50 dark:bg-success-500/10','color'=>'text-success-600 dark:text-success-400'],['label'=>'Active Agreements','value'=>'22','bg'=>'bg-warning-50 dark:bg-warning-500/10','color'=>'text-warning-600 dark:text-warning-400']] as $c)
...
<div class="rounded-2xl bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-gray-800 shadow-theme-xs overflow-hidden">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-6 py-4 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Partner Organizations</h3>
    ...
    <div class="px-6 py-3 border-b border-gray-100 dark:border-gray-800">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input type="text" placeholder="Search organizations..." class="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-transparent text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 transition">
        ...
        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
            <thead class="bg-gray-50 dark:bg-white/[0.02]">
                <tr>
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Location</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Members</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @php
                    ...
                    $typeColor = ['School'=>'bg-brand-50 text-brand-700 dark:bg-brand-500/10 dark:text-brand-400','Clinic'=>'bg-success-50 text-success-700 dark:bg-success-500/10 dark:text-success-400','NGO'=>'bg-purple-50 text-purple-700 dark:bg-purple-500/10 dark:text-purple-400','Government'=>'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400','Corporate'=>'bg-gray-100 text-gray-600 dark:bg-white/5 dark:text-gray-400'];
                @endphp
                @foreach($orgs as $org)
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    <td class="px-5 py-3">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-xs font-bold flex-shrink-0">{{ substr($org['name'],0,1) }}</div>
                    ...
                    <td class="px-5 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $org['loc'] }}</td>
                    <td class="px-5 py-3 text-sm font-semibold text-gray-900 dark:text-white">{{ $org['members'] }}</td>
                    <td class="px-5 py-3"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $org['status'] === 'active' ? 'bg-success-50 text-success-700 dark:bg-success-500/10 dark:text-success-400' : 'bg-warning-50 text-warning-700 dark:bg-warning-500/10 dark:text-warning-400' }}">{{ ucfirst($org['status']) }}</span></td>
                    <td class="px-5 py-3 text-right">
                        <a href="{{ route('admin.organizations.show', 1) }}" class="p-1.5 rounded-lg text-gray-400 hover:bg-brand-50 hover:text-brand-600 dark:hover:bg-brand-500/10 dark:hover:text-brand-400 transition-colors inline-flex">
