                        ...
                        <div class="space-y-2">
                            @foreach($lessonTopics as $index => $topic)
                                <a href="{{ route('learner.lessons.show', ['lesson' => $lesson->id, 'topic' => $index]) }}" 
                                   class="block p-3 rounded-lg transition {{ $currentTopicIndex === $index && !request()->has('quiz') ? 'bg-blue-500 text-white' : 'bg-gray-50 hover:bg-gray-100' }}">
                                    <div class="flex items-start gap-2">
                                        @if(in_array($topic->id, $completedTopicIds))
                                            <svg class="w-5 h-5 flex-shrink-0 mt-0.5 {{ $currentTopicIndex === $index && !request()->has('quiz') ? 'text-white' : 'text-green-500' }}" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                            </svg>
                                        @else
                                            <div class="w-5 h-5 flex-shrink-0 mt-0.5 rounded-full border-2 {{ $currentTopicIndex === $index && !request()->has('quiz') ? 'border-white' : 'border-gray-300' }}"></div>
                                        @endif
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium truncate">{{ $topic->title }}</p>
                                            <p class="text-xs {{ $currentTopicIndex === $index && !request()->has('quiz') ? 'text-blue-100' : 'text-gray-500' }} mt-0.5">
                                                {{ $topic->duration }} min
                                            </p>
                                        </div>
                                    </div>
