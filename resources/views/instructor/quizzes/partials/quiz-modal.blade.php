    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-theme-md border border-gray-200 dark:border-gray-800 max-w-2xl w-full max-h-[90vh] overflow-y-auto"
        ...
        <form method="POST" action="{{ route('instructor.quizzes.store') }}" class="p-6">
            ...
            <div class="mb-6 p-4 rounded-xl bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20">
                ...
                <p class="text-xs text-brand-600 dark:text-brand-400 mb-4">Select a module, then optionally narrow down to a specific lesson within it.</p>

                <!-- Step 1: Module -->
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        Module
                        <span class="text-xs font-normal text-gray-500 dark:text-gray-400">(quiz appears after all lessons)</span>
                    </label>
                    <select name="module_id"
                            x-model="selectedModule"
                            @change="onModuleChange()"
                            class="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition">
                ...
                <div x-show="selectedModule !== ''" x-cloak>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        Specific Lesson
                        <span class="text-xs font-normal text-gray-500 dark:text-gray-400">(optional — leave blank to attach to the whole module)</span>
                    </label>
                    ...
                    <p class="mt-1.5 text-xs text-brand-700 dark:text-brand-400" x-show="selectedLesson !== ''" x-cloak>
                ...
                @error('module_id')
                    <p class="mt-2 text-xs text-error-600 dark:text-error-400">{{ $message }}</p>
                @enderror
            </div>
