<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-between items-center mt-4">
            ...
            <a href="{{ route('instructor.lessons.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Create New Lesson</a>
        </div>
    </x-slot>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-sm text-gray-500">{{ Str::limit($lesson->description ?? '', 80) }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($lesson->module)
                                        {{ $lesson->module->title }}
                                    @else
                                        <span class="text-red-500">No Module</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">{{ $lesson->duration }} min</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('instructor.lessons.edit', $lesson) }}" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 mr-2">Edit</a>
