<section class="w-full lg:w-80 xl:w-96 border-r border-gray-200 bg-white" data-chat-conversation-list>
    <div class="p-4 border-b border-gray-100 space-y-3">
        ...
        <template x-if="$store.chat.currentUserRole !== 'admin' && $store.chat.discovery.supportAdmin">
            <button
                type="button"
                class="inline-flex w-full items-center justify-center rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800 hover:bg-blue-100"
    ...
    <div class="max-h-[30vh] overflow-y-auto border-b border-gray-100">
        <p class="px-4 pt-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-500">Recent Conversations</p>

        <template x-if="$store.chat.filteredConversations().length < 1">
            <div class="px-4 pb-4 text-xs text-gray-500">No conversations yet. Start one from contacts below.</div>
        </template>

        <template x-for="conversation in $store.chat.filteredConversations()" :key="conversation.id">
            <button
                type="button"
                class="flex w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50"
                :class="$store.chat.activeConversationId === conversation.id ? 'bg-blue-50/80' : ''"
            ...
            >
                ...
                <span
                    class="inline-flex min-w-5 items-center justify-center rounded-full bg-blue-600 px-1.5 py-0.5 text-[10px] font-semibold text-white"
